...
export default function HunterWishlistPage() {
  ...
  if (loading) {
    return (
      <div className="flex h-screen items-center justify-center bg-off-white dark:bg-stone-950">
  ...
  return (
    <div className="min-h-screen bg-off-white dark:bg-stone-950 pb-20 transition-colors">
      
      {/* Header */}
      <div className="bg-olive dark:bg-stone-900 py-10 border-b-4 border-kalahari">
        <div className="max-w-6xl mx-auto px-6">
          <Button 
            variant="ghost" 
            onClick={() => router.push("/hunter/dashboard")}
            className="text-off-white hover:text-kalahari hover:bg-white/10 dark:hover:bg-black/20 -ml-4 mb-4"
          >
      ...
      <div className="max-w-6xl mx-auto px-6 mt-8">
        {savedHunts.length === 0 ? (
          <div className="text-center py-20 bg-white dark:bg-stone-900 border-2 border-dashed border-kalahari/30 dark:border-kalahari/50 rounded-2xl shadow-sm transition-colors">
            ...
            <p className="text-olive/80 dark:text-off-white/70 mt-2 mb-8 max-w-md mx-auto font-medium">
              You haven't saved any hunts yet. Browse the marketplace and click the bookmark icon to save hunts for later.
            </p>
            <Link href="/">
              <Button className="bg-olive dark:bg-kalahari hover:bg-olive/90 dark:hover:bg-kalahari/90 text-kalahari dark:text-olive font-black px-8 h-14 text-lg rounded-xl shadow-md transition-colors">
                Explore Marketplace
              </Button>
            </Link>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {savedHunts.map((hunt) => (
              <Card key={hunt.id} className="h-full overflow-hidden bg-white dark:bg-stone-900 border-2 border-kalahari/20 dark:border-kalahari/40 hover:border-kalahari dark:hover:border-kalahari transition-colors group shadow-sm hover:shadow-md flex flex-col relative">
                ...
                <button 
                  ...
                  className="absolute top-3 right-3 z-20 bg-white/90 dark:bg-black/60 hover:bg-red-50 dark:hover:bg-red-900/50 text-olive dark:text-off-white p-2 rounded-full shadow-md backdrop-blur-sm transition-colors border border-kalahari/20 dark:border-kalahari/40"
                  title="Remove from wishlist"
                >
                  {removingId === hunt.id ? (
                    <Loader2 className="h-5 w-5 animate-spin text-red-600 dark:text-red-400" />
                  ) : (
                    <Trash2 className="h-5 w-5 hover:text-red-600 dark:hover:text-red-400 transition-colors" />
                  )}
                </button>

                <Link href={`/hunts/${hunt.id}`} className="flex-1 flex flex-col">
                  <div className="h-48 relative bg-kalahari/10 dark:bg-kalahari/20 overflow-hidden">
                    {hunt.coverImage ? (
                  ...
                  <CardContent className="p-5 flex-1 flex flex-col">
                    <div className="flex items-center gap-2 text-xs font-bold text-olive dark:text-off-white/60 uppercase tracking-wider mb-2">
                      <MapPin className="h-3 w-3 text-kalahari" /> {hunt.location}
                    </div>
                    ...
                    <div className="flex items-center justify-between mt-auto pt-4 border-t border-kalahari/10 dark:border-kalahari/20">
                      <span className="font-bold text-olive/80 dark:text-off-white/70">{hunt.duration} Days</span>
