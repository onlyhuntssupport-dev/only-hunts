...
export default function EditHuntPage({ params }: Props) {
    ...
    const fetchHuntData = async () => {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          if (data.outfitterId !== auth.currentUser.uid) {
          ...
          setFormData({
            title: data.title || "",
            price: Number(data.price || data.basePrice || 3500),
          ...
          setImagePreview(data.coverImage || data.imageUrl || null);
          setExistingCoverUrl(data.coverImage || data.imageUrl || "");

          // Parse Primary Species
          if (data.primarySpeciesArray) {
            setPrimarySpecies(data.primarySpeciesArray);
          } else if (data.primarySpecies) {
            setPrimarySpecies(data.primarySpecies.split(',').map((s: string) => s.trim()));
          }

          // Parse Additional Species
          if (data.additionalSpeciesArray) {
            setAdditionalSpecies(data.additionalSpeciesArray);
          }

          // Parse Inclusions
          const parseList = (raw: string | string[], standardList: string[]) => {
            if (!raw) return { matched: [], unmatched: "" };
            const strArray = Array.isArray(raw) ? raw : raw.split('\n');
            const lines = strArray.map((l: string) => l.replace('• ', '').trim()).filter(Boolean);
            const matched = lines.filter((l: string) => standardList.includes(l));
            const unmatched = lines.filter((l: string) => !standardList.includes(l)).join(', ');
            return { matched, unmatched };
          };

          const incParsed = parseList(data.includedItems, COMMON_INCLUSIONS);
          setIncludedItems(incParsed.matched);
          setOtherInclusions(incParsed.unmatched);

          const excParsed = parseList(data.excludedItems, COMMON_EXCLUSIONS);
          setExcludedItems(excParsed.matched);
          setOtherExclusions(excParsed.unmatched);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error("Error updating hunt:", err);
  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto pb-12 transition-colors duration-300">
      
      <div className="flex items-center gap-4 border-b-2 border-kalahari/30 dark:border-kalahari/20 pb-6">
        <Link href="/outfitter/dashboard/hunts">
          <Button variant="outline" size="icon" className="h-10 w-10 rounded-full border-kalahari/50 dark:border-kalahari/30 text-olive dark:text-off-white hover:bg-kalahari/20 dark:hover:bg-kalahari/20 dark:bg-transparent">
            <ArrowLeft className="h-5 w-5" />
          </Button>
        </Link>
        <div>
          <h1 className="text-3xl font-headline font-bold text-olive dark:text-off-white tracking-tight transition-colors">Edit Package</h1>
          <p className="text-olive/70 dark:text-off-white/70 font-medium transition-colors">Update details for "{formData.title}"</p>
        </div>
      </div>

      {error && (
        <div className="bg-red-50 dark:bg-red-900/30 text-red-800 dark:text-red-300 p-4 rounded-lg border border-red-200 dark:border-red-800/50 font-bold shadow-sm transition-colors">
          {error}
        </div>
      )}

      <form onSubmit={handleSubmit} className="bg-white dark:bg-olive border-2 border-kalahari/30 dark:border-kalahari/40 rounded-xl p-6 md:p-10 shadow-sm space-y-12 transition-colors">
        ...
        <div className="space-y-8">
          ...
          <div 
            ...
            className={`border-2 rounded-xl p-5 md:p-6 flex flex-col md:flex-row md:items-center justify-between cursor-pointer transition-all ${
              isSpecialOffer ? 'bg-orange-50 dark:bg-orange-950/40 border-orange-400 dark:border-orange-500 shadow-md' : 'bg-off-white dark:bg-black/20 border-kalahari/30 dark:border-kalahari/40 hover:border-kalahari/60 dark:hover:border-kalahari/60'
            }`}
          >
          ...
          <div>
            ...
            <Input name="title" required value={formData.title} onChange={handleTextChange} placeholder="e.g. 5-Day Premium Kudu Safari" className="h-14 bg-white dark:bg-black/40 border-kalahari/50 dark:border-kalahari/30 focus-visible:ring-olive dark:focus-visible:ring-kalahari text-olive dark:text-off-white dark:placeholder:text-off-white/40 font-bold text-xl transition-colors" />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div>
              ...
              <div className="flex items-center h-14 bg-white dark:bg-black/40 border-2 border-kalahari/50 dark:border-kalahari/30 rounded-md overflow-hidden focus-within:ring-2 focus-within:ring-olive dark:focus-within:ring-kalahari focus-within:border-transparent transition-colors">
                <button type="button" onClick={() => setFormData(p => ({ ...p, price: Math.max(0, p.price - 500) }))} className="w-14 shrink-0 h-full bg-off-white dark:bg-black/60 hover:bg-kalahari/20 dark:hover:bg-kalahari/20 flex items-center justify-center text-olive dark:text-off-white border-r-2 border-kalahari/50 dark:border-kalahari/30 transition-colors">
                ...
                />
                <button type="button" onClick={() => setFormData(p => ({ ...p, price: p.price + 500 }))} className="w-14 shrink-0 h-full bg-off-white dark:bg-black/60 hover:bg-kalahari/20 dark:hover:bg-kalahari/20 flex items-center justify-center text-olive dark:text-off-white border-l-2 border-kalahari/50 dark:border-kalahari/30 transition-colors">
            ...
            <div>
              ...
              <div className="flex items-center h-14 bg-white dark:bg-black/40 border-2 border-kalahari/50 dark:border-kalahari/30 rounded-md overflow-hidden focus-within:ring-2 focus-within:ring-olive dark:focus-within:ring-kalahari focus-within:border-transparent transition-colors">
                <button type="button" onClick={() => setFormData(p => ({ ...p, duration: Math.max(1, p.duration - 1) }))} className="w-14 shrink-0 h-full bg-off-white dark:bg-black/60 hover:bg-kalahari/20 dark:hover:bg-kalahari/20 flex items-center justify-center text-olive dark:text-off-white border-r-2 border-kalahari/50 dark:border-kalahari/30 transition-colors">
                  <Minus className="h-5 w-5" />
                </button>
                <input 
                  type="number" 
          ...
          <div className="bg-off-white dark:bg-black/20 p-6 rounded-xl border border-kalahari/20 dark:border-kalahari/30 transition-colors">
          ...
          <div>
            ...
            <Textarea name="description" required value={formData.description} onChange={handleTextChange} rows={6} placeholder="Describe the daily itinerary, accommodation style, and what to expect..." className="bg-white dark:bg-black/40 border-kalahari/50 dark:border-kalahari/30 focus-visible:ring-olive dark:focus-visible:ring-kalahari text-olive dark:text-off-white dark:placeholder:text-off-white/40 font-medium text-base resize-y transition-colors" />
          </div>
        </div>
        ...
        <div className="space-y-8 pt-8 border-t-2 border-kalahari/20 dark:border-kalahari/30">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
            ...
            {/* Included Section */}
            <div className="bg-green-50/50 dark:bg-green-950/30 p-6 rounded-xl border-2 border-green-200 dark:border-green-900/50 transition-colors">
              ...
              <Input value={otherInclusions} onChange={(e) => setOtherInclusions(e.target.value)} placeholder="Other specific inclusions..." className="bg-white dark:bg-black/40 border-green-300 dark:border-green-800 focus-visible:ring-green-600 dark:focus-visible:ring-green-500 text-green-900 dark:text-green-100 dark:placeholder:text-green-100/40 transition-colors" />
            </div>

            {/* Excluded Section */}
            <div className="bg-red-50/50 dark:bg-red-950/30 p-6 rounded-xl border-2 border-red-200 dark:border-red-900/50 transition-colors">
              ...
              <Input value={otherExclusions} onChange={(e) => setOtherExclusions(e.target.value)} placeholder="Other specific exclusions..." className="bg-white dark:bg-black/40 border-red-300 dark:border-red-800 focus-visible:ring-red-600 dark:focus-visible:ring-red-500 text-red-900 dark:text-red-100 dark:placeholder:text-red-100/40 transition-colors" />
            </div>
          </div>

          {/* Additional Species Pricing Matrix */}
          <div className="bg-white dark:bg-black/20 p-6 rounded-xl border-2 border-kalahari/30 dark:border-kalahari/30 transition-colors">
            ...
            <div className="space-y-3">
              ...
              {additionalSpecies.map((species) => (
                <div key={species.name} className="flex items-center gap-4 bg-off-white dark:bg-black/40 p-3 rounded-lg border border-kalahari/20 dark:border-kalahari/30 animate-in fade-in duration-200 transition-colors">
                  ...
                  <div className="relative w-32 md:w-48">
                    ...
                    <Input 
                      ...
                      className="pl-8 bg-white dark:bg-black/50 border-kalahari/30 dark:border-kalahari/40 focus-visible:ring-olive dark:focus-visible:ring-kalahari text-olive dark:text-off-white dark:placeholder:text-off-white/40 font-bold transition-colors" 
                    />
                  </div>
        ...
        <div className="pt-8 border-t-2 border-kalahari/20 dark:border-kalahari/30 flex justify-end transition-colors">
          <Button type="submit" disabled={saving} className="bg-olive dark:bg-kalahari hover:bg-olive/90 dark:hover:bg-kalahari/90 text-kalahari dark:text-olive dark:text-off-white font-black text-lg h-14 px-12 shadow-xl hover:shadow-2xl hover:-translate-y-1 transition-all flex items-center gap-2 w-full md:w-auto rounded-full">
            ...
            {saving ? "Updating Package..." : "Update Package"}
          </Button>
        </div>
      </form>
    </div>
